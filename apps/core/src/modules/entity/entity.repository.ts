...
export class EntityRepository extends BaseRepository<"entities"> {
  ...
  // an "at or under any of these nodes" predicate against an entity's own
  // joined location (see #joinLocationHierarchy's `loc` alias).
  async #resolveLocationPaths(
    trx: { selectFrom: (table: "locations") => any },
  ...
  async #generateQueryWhereClause(c: Context, query, params: GetEntitiesQueries) {
    const {
      keyword,
      id_satu_sehat,
      type_ids,
    ...
    // (see #joinLocationHierarchy) so that the `loc` alias exists.
    // An entity matches if its own location is at-or-under ANY of the
    // selected location_ids' subtrees (OR'd across every selected id).
    if (location_ids?.length > 0) {
      const paths = await this.#resolveLocationPaths(c.var.trx, location_ids)
  ...
  async findAllSearchableAndStreamable(
    ...
    const locationPaths = await this.#resolveLocationPaths(
      c.var.trx,
      params.location_ids ?? []
    )
