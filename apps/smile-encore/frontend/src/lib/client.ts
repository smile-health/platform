...
export function Environment(name: string): BaseURL {
    return `https://${name}-kz2pw.encr.app`
}
...
/**
 * Client is an API client for the kz2pw Encore application.
 */
...
export namespace materials {
    ...
    export class ServiceClient {
        ...
        public async create(params: CreateMaterialRequest): Promise<Material> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("POST", `/materials`, JSON.stringify(params))
            return await resp.json() as Material
        }

        public async get(id: number): Promise<Material> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("GET", `/materials/${encodeURIComponent(id)}`)
            return await resp.json() as Material
        }

        public async list(params: ListMaterialsParams): Promise<ListMaterialsResponse> {
            ...
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("GET", `/materials`, undefined, {query})
            return await resp.json() as ListMaterialsResponse
        }
        ...
        public async remove(id: number): Promise<void> {
            await this.baseClient.callTypedAPI("DELETE", `/materials/${encodeURIComponent(id)}`)
        }
    ...
    status?: number
            ...
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI("PUT", `/materials/${encodeURIComponent(id)}`, JSON.stringify(params))
            return await resp.json() as Material
        }
    }
}
...
class BaseClient {
    ...
    constructor(baseURL: string, options: ClientOptions) {
        ...
        if (!BROWSER) {
            this.headers["User-Agent"] = "kz2pw-Generated-TS-Client (Encore/v1.57.13)";
        }
