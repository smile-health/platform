import { getExportPartnershipVehicle } from '@/services/download-excel';
import { GetExcelExportParams } from '@/types/download-excel';
import { toast } from '@repo/ui/components/toast';
import { useMutation } from '@tanstack/react-query';
import { useTranslation } from 'react-i18next';

export const useDownloadVehice = () => {
  const { t } = useTranslation(['common', 'logbook']);

  const {
    mutateAsync: downloadVehice,
    isPending: isLoading,
    error,
  } = useMutation({
    mutationFn: (params: GetExcelExportParams) =>
      getExportPartnershipVehicle(params),
    onSuccess: () => {
      toast.success({
