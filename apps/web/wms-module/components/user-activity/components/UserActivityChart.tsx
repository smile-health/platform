...
import { PieChart } from '@repo/ui/components/chart';
import {
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuRoot,
...
import { DOWNLOAD_EXTENSIONS } from '../constants/user-activity.constant';
...
const UserActivityChart = forwardRef(function UserActivityChart(
  ...
  const provinceLabel = filter?.provinceActivity?.label;
  const regencyLabel = filter?.regencyActivity?.label;
  const entityLabel = filter?.healthcareFacilityActivity?.label;
