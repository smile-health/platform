...
import { requestReadById } from '@/services/notification';
...
export const useRequestApprovalHandler = ({
  ...
  const [selectedItem, setSelectedItem] = useState<TNotification | null>(null);
  ...
  const {
    ...
    mutationFn: async ({
      ...
      if (item.readAt === null) {
        await requestReadById(item.id); // Mark as read
      }
    },
    onSuccess: (_data, variables) => {
      const status = variables?.status;
      const name = variables.item.userName ?? '';
  ...
  const handleRequestApprovalClick = (item: TNotification) => {
