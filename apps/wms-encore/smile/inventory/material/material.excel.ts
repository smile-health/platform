...
import * as materialTypeRepo from "../material-type/material-type.repository";
import * as materialSubtypeRepo from "../material-subtype/material-subtype.repository";
import * as materialUnitRepo from "../material-unit/material-unit.repository";
import * as materialRelationRepo from "../material-relation/material-relation.repository";
...
export async function exportMaterials(materials: MaterialRow[]): Promise<Buffer> {
  ...
  const [levels, types, subtypes, units, parentRelationsByChild, programsByMaterial] = await Promise.all([
    materialLevelRepo.findAll(),
    materialTypeRepo.findAll(),
    materialSubtypeRepo.findAll(),
    materialUnitRepo.findAll(),
    materialRelationRepo.findByChildIds(materialIds),
