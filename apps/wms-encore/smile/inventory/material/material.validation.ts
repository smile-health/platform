...
const typeExists = existsCheck<MaterialRequest>({
  ...
  checkExists: async (id) => !!(await materialTypeRepo.findById(id)),
...
const subtypeBelongsToType: ValidatorNode<MaterialRequest> = when(
  ...
  async (ctx) => {
    const subtype = await materialSubtypeRepo.findById(ctx.data.material_subtype_id!);
    if (!subtype) {
...
const parentHierarchyValid: ValidatorNode<MaterialRequest> = when(
  ...
  async (ctx) => {
    ...
    const levels = await materialLevelRepo.findAll();
