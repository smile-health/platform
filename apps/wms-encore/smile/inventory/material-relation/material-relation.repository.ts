...
export async function findByChildIds(childMaterialIds: number[]): Promise<MaterialRelationRow[]> {
  if (!childMaterialIds.length) return [];
  return db
    .selectFrom("material_relations")
    .selectAll()
    .where("child_material_id", "in", childMaterialIds)
