...
import type {
  EnvironmentalHealthDocumentData,
...
export class EnvironmentalHealthPdfGenerator {
  private doc!: jsPDF
  ...
  private buildDocument(data: EnvironmentalHealthDocumentData): void {
    const { tr } = this
    ...
    this.addLabelValueTable([
      ...
      [
        ...
        formatDate(data.received_date, this.timezone),
      ],
      [
        tr("collection_date", "Tanggal Pengambilan"),
        formatDate(data.collection_date, this.timezone),
      ],
      ...
      [
        tr("lab_result_status", "Status Hasil Lab"),
      ...
      [
        ...
        formatDate(data.test_start_date, this.timezone),
      ],
      [
        tr("test_end_date", "Tanggal Selesai Uji"),
        formatDate(data.test_end_date, this.timezone),
      ],
    ])
    ...
    this.addLabelValueTable([
      ...
      [
        ...
        formatDate(data.ikl_test_date, this.timezone),
      ],
      [
    ...
    if (data.distribution_details && data.distribution_details.length > 0) {
      this.currentY += 5
      this.addSectionTitle(
        tr("distribution_details_title", "Detail Distribusi Akhir")
      )
  ...
  private renderParameterCategoryItems(
    items: EnvironmentalHealthDocumentData["parameter_category_items"]
  ): void {
  ...
  private renderDistributionDetailsTable(
    ...
    const tanggalDistribusi = [
      ...
      ...details.map(
        (d) => dayjs(d.actual_transaction_date).format("DD MMM YYYY") ?? "-"
      ),
    ]
    const jamDistribusi = [
      tr("jam_distribusi", "Jam Distribusi"),
      ...details.map(
        (d) => dayjs(d.actual_transaction_date).format("HH:mm") ?? "-"
      ),
    ]
    const manufacture = [
    ...
    const productionDate = [
      ...
      ...details.map((d) =>
        formatDate(d.batch?.production_date, this.timezone)
      ),
    ]
    const expirationDate = [
      tr("expiration_date", "Tanggal Kadaluarsa"),
      ...details.map((d) =>
        formatDate(d.batch?.expiration_date, this.timezone)
      ),
    ]
