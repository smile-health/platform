...
export async function up(db: Kysely<Database>): Promise<void> {
  await sql`TRUNCATE TABLE ${sql.table("ws_material_targets")}`.execute(db)
  ...
  if (insertsNonBiasPrimary.length > 0) {
    await db
      .insertInto("ws_material_targets")
      .values(insertsNonBiasPrimary)
      .execute()
  }
  ...
  if (insertsNonBiasAdditional.length > 0) {
    await db
      .insertInto("ws_material_targets")
      .values(insertsNonBiasAdditional)
      .execute()
  }
