...
/**
 * Seed ws_bmhp_material_details: bridge antara bmhp_materials dan materials level 2 (Merk)
 * Data dari Master_Data_Product_Variant.xlsx kolom A (BMHP) → kolom B (Merk)
 * Total: 147 unique pairs
 *
 * Dependency:
 * - bmhp_materials harus sudah ada (migration 1773187200003)
 * - materials level 2 (Merk) harus sudah ada (migration 1773187200004)
 *
 * material_id = materials.id (level 2), bukan material_workspaces.id
 */
...
export async function up(db: Kysely<any>): Promise<void> {
  // Lookup bmhp_materials by name → id
  ...
  const bmhpIdByName = new Map<string, number>(
    bmhpRows.map((r) => [r.name.toLowerCase().trim(), Number(r.id)])
  )

  // Lookup materials level 2 by name → id
  ...
  // Existing details (idempotent: cek by bmhp_material_id + material_id)
  ...
  for (const pair of BMHP_MERK_PAIRS) {
    const bmhpId = bmhpIdByName.get(pair.bmhp.toLowerCase().trim())
    const materialId = materialIdByName.get(pair.merk.toLowerCase().trim())

    if (!bmhpId || !materialId) continue

    const key = `${bmhpId}|${materialId}`
  ...
  for (let i = 0; i < toInsert.length; i += BATCH) {
    await db.insertInto("ws_bmhp_material_details").values(toInsert.slice(i, i + BATCH)).execute()
  }
}
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  const bmhpIds = BMHP_MERK_PAIRS
    .map((p) => bmhpIdByName.get(p.bmhp.toLowerCase().trim()))
    .filter((id): id is number => id !== undefined)

  if (bmhpIds.length > 0) {
    await db
      .deleteFrom("ws_bmhp_material_details")
      .where("bmhp_material_id", "in", [...new Set(bmhpIds)])
      .execute()
  }
}
