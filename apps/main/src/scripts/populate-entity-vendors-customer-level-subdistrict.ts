...
// Constants untuk activity IDs - bisa dipindah ke config file
const ACTIVITY_IDS = [18, 19, 20, 21] as const
const MATERIAL_LEVEL_ID = 2

export const populateEntityMaterialsAndVendors = async ({
  provinceId,
  regencyId,
  entityTagId,
  programId,
  ...
  try {
    console.info("Starting entity, vendor, and material population...")
    ...
    await db.transaction().execute(async (trx) => {
      ...
      const globalEntities = await trx
        ...
        .where((eb) =>
          eb.and([
            ...
            eb.or([
              eb("we.village_id", "is", null),
              eb("we.village_id", "=", ""),
            ]),
          ])
        )
        .where("we.entity_tag_id", "=", entityTagId)
        .execute()
      ...
      const newCvaValues = ACTIVITY_IDS.flatMap((activityId) =>
        allCustomerVendors
          .map((cv) => ({
      ...
      const newEntityActivities = ACTIVITY_IDS.flatMap((activityId) =>
        programEntityIds
          .map((entityId) => ({
            entity_id: entityId,
            activity_id: activityId,
      ...
      if (materials.length === 0) {
        ...
        // Generate all combinations untuk PROGRAM entities (bukan global entities)
        const newEmaValues = programEntityIds.flatMap((programEntityId) =>
          ACTIVITY_IDS.flatMap((activityId) =>
            materialIds
              .map((materialId) => ({
        ...
        const vendorEmaValues = ACTIVITY_IDS.flatMap((activityId) =>
          materialIds
            .map((materialId) => ({
              entity_id: vendorId,
              activity_id: activityId,
