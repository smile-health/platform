...
const rawSlave = new DatabaseManager<Database>(
  dialect,
  env.APP_DEBUG
).getDB()
...
if (slave) {
  console.log(`🔌 [WAREHOUSE-SLAVE] ClickHouse slave database initialized with URL: ${env.CLICKHOUSE_DATABASE_URL ? '[CONFIGURED]' : '[NOT_CONFIGURED]'}`)
  connectionStatus = env.CLICKHOUSE_DATABASE_URL ? 'connected' : 'disconnected'
  lastConnectionAttempt = new Date()
} else {
  console.log(`⚠️ [WAREHOUSE-SLAVE] ClickHouse slave database not configured - CLICKHOUSE_DATABASE_URL is missing`)
...
export const executeUpdateQuery = async (query: CompiledQuery) => {
  ...
  if (env.ENABLE_CLICKHOUSE_TRACING) {
    return await clickhouseTracer.traceQuery('UPDATE', `ALTER TABLE ${formattedTableName} UPDATE ${setClause}`, parameters as any[])(() =>
      slave?.getClient().executeQuery(
        CompiledQuery.raw(
    ...
    return await slave?.getClient().executeQuery(
      CompiledQuery.raw(
        `ALTER TABLE ${formattedTableName} UPDATE ${setClause}`,
        parameters as unknown[]
      )
...
export async function healthCheck() {
  ...
  try {
    ...
    console.error(`❌ [WAREHOUSE-SLAVE] Health check failed:`, (error as Error).message)
    return {
      healthy: false,
      service: 'clickhouse-slave',
      status: 'error',
