...
import { ConsumptionSupplyModule } from "./consumption-supply.module.js"
import { ConsumptionSupplyQueryParamsSchema } from "./consumption-supply.schema.js"

export class ConsumptionSupplyController extends BaseController {
  constructor(
    private readonly module: ConsumptionSupplyModule,
    private readonly roleMiddleware: RoleMiddleware
  ) {
    super("consumption_supply")
  }

  getRoutes(): Hono {
    const router = new Hono()

    // Apply role-based access control - same as order-difference
    router.use(
    ...
    router.get(
      "/review",
      this.validateRequest("query", ConsumptionSupplyQueryParamsSchema),
      async (c) => {
