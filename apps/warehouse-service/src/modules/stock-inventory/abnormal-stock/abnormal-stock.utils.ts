...
/**
 * Calculate abnormal stock value based on information type
 * Used by abnormal-stock module - ported from old getStockValue function
 */
export function getAbnormalStockValue(
 ...
 */
export function processAbnormalStockData(
  ...
  options: {
    ...
    abnormalInformationType?: string
  } = {}
      ...
      return items.map((entity) => {
        ...
        const periodData: StockInventoryResultItem[] = materials.map(
          (material) => {
            ...
            const value = getAbnormalStockValue(
              materialData,
              divider,
              abnormalInformationType
            )
...
/**
 * Generate type array for abnormal stock responses
 * Used by abnormal-stock module
 */
export function generateAbnormalStockTypes(): Array<{
  key: string
  label: string
}> {
  return [
