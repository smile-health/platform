...
> Merujuk `biofarma.cron.ts`, `biofarma.gateway.ts`, `biofarma.repository.ts`, dan `sync_orders.ts` yang semuanya sudah dihapus.
> Disimpan sebagai rujukan historis saja.
...
| File | Purpose |
|------|---------|
| `apps/main/src/modules/order-integration/biofarma/biofarma.cron.ts` _(dihapus)_ | `BiofarmaCron` — core sync logic (syncOrders, syncDashboard) |
| `apps/main/src/modules/order-integration/biofarma/biofarma.gateway.ts` _(dihapus)_ | `BiofarmaGateway` — HTTP client for Biofarma API and SMILE API |
| `apps/main/src/modules/order-integration/biofarma/biofarma.repository.ts` _(dihapus)_ | `BiofarmaRepository` — DB reads/writes |
| `apps/main/src/scripts/cron/biofarma/sync_orders.ts` _(dihapus)_ | CLI entry point — sets up transaction context and invokes `BiofarmaCron` |
...
Follow [adding-new-event.md](../../architecture/interop-layer/adding-new-event.md) Scenario A. The steps below provide Biofarma-specific SQL values. **Replace placeholder adapter URLs with real targets once downstream systems are confirmed.**

### Step 1 — Confirm the RabbitMQ Topic Name

Topic: **`biofarma.order.created`**

Published inside `BiofarmaCron.syncOrders()` `apps/main/src/modules/order-integration/biofarma/biofarma.cron.ts` _(dihapus)_ once per DO after `createSmileOrder()` succeeds. **Publish timing: 07:00 WIB daily** (cron: `0 0 * * *` UTC).
...
Add to [apps/interop-service/db-scripts/route-mapping-insert.sql](https://github.com/smile-health/platform/blob/main/apps/interop-service/db-scripts/route-mapping-insert.sql).
...
Add to [apps/openhim-mediators/rule-router/db-scripts/routing-rules-insert.sql](https://github.com/smile-health/platform/blob/main/apps/openhim-mediators/rule-router/db-scripts/routing-rules-insert.sql).
...
- [Architecture](../../architecture/interop-layer/architecture.md) — Full interop layer architecture
- [Adding a New Event](../../architecture/interop-layer/adding-new-event.md) — Generic step-by-step guide
- [Configuration Reference](../../architecture/interop-layer/configuration-reference.md) — Environment variables and DB schemas
- [Operations](../../architecture/interop-layer/operations.md) — Running locally, Docker Compose, Kubernetes
