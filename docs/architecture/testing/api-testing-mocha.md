...
> **Cakupan sempit.** Panduan ini menulis Mocha + Chai + NYC sebagai stack default. Kenyataan sekarang:
>
> | Lapis | Tooling | Di mana |
> |---|---|---|
> | Unit & integrasi | **Vitest** | semua service — lihat [`vitest.workspace.ts`](https://github.com/smile-health/platform/blob/main/vitest.workspace.ts) |
> | API end-to-end | **Playwright** | [`packages/global-tests/`](https://github.com/smile-health/platform/blob/main/packages/global-tests) — lihat [panduan Playwright](./warehouse-playwright-api-tests.md) |
> | Mocha + Chai | tersisa | hanya script `test:mocha` dan `test:api` di [`apps/core`](https://github.com/smile-health/platform/blob/main/apps/core/package.json); pola file-nya `src/tests/**/*.test.ts`, bukan `/tests/api/*.test.js` seperti di bawah |
>
> Layout `.nycrc` dan `/tests/api/*.test.js` yang dipreskripsikan dokumen ini tidak ada di repo.
> Untuk test baru, mulai dari Vitest atau Playwright. Dokumen ini dipertahankan untuk
> menjelaskan suite Mocha yang masih tersisa di `apps/core`.
