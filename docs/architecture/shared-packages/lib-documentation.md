...
> **Cakupan parsial.** Sembilan modul yang didokumentasikan di sini semuanya masih ada.
> Yang belum tercakup: `cache.ts`, `clamav.ts`, `masking.ts`, `feature-flags/`, `novu/`,
> `rabbitmq/`, `mail/`, dan `cron/` — lihat [`packages/lib/`](https://github.com/smile-health/platform/blob/main/packages/lib).
