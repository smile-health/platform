...
> **Belum ada helper terpusat.** Bagian di bawah merujuk `packages/lib/infrastructure-monitor.ts`
> dan `examples/infrastructure-monitoring-usage.ts` (`setupAllServicesMonitoring`,
> `generateInfrastructureReport`, `createHealthCheckEndpoint`). Tidak satu pun ada di repo —
> perlakukan sebagai desain yang diusulkan, bukan API yang bisa dipakai.
> Yang benar-benar tersedia hari ini adalah `healthCheck()` dan `getConnectionStatus()`
> per-modul infrastruktur, sebagaimana dicontohkan tepat di bawah ini.
