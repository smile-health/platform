...
> **`apps/sync-service` sudah tidak ada di repo ini.** Instruksi `cd apps/sync-service && npm run migrate:all`
> di bawah, dan rujukan `apps/sync-service/src/scripts/data-migration/const.ts` di
> [constants-mapping.md](./constants-mapping.md), tidak lagi bisa dijalankan apa adanya.
> Query SQL, mapping tabel, dan validation queries di folder ini tetap dipertahankan
> karena masih dipakai manual untuk migrasi SMILE 3.0 → 5.0.
