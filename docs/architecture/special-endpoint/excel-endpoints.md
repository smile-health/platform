...
> **Dump point-in-time.** Berkas `.yml` di folder ini di-generate manual dari `wire.ts` dan
> tidak ada script regenerasinya di repo, jadi drift tidak terpantau. `clickhouse-list.yml`
> masih memakai prefix `/main/...` yang tidak cocok dengan mounting `wire.ts` sekarang.
> Verifikasi terhadap `wire.ts` sebelum dipakai sebagai acuan.
