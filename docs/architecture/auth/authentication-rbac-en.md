...
> **Catatan contoh.** Client-role `siha`, `sitb`, dan `din` yang dipakai sebagai contoh
> di bawah sudah tidak punya modul backing di `apps/` — modulnya dicabut saat pembersihan
> Indonesia-only. Mekanisme RBAC-nya sendiri tetap berlaku; perlakukan nama-nama itu
