...
export class MultiSheetZipExporter {
  ...
  public async addRows(
    ...
    if (Array.isArray(rows)) {
      ...
      for (const row of rows) {
        // ✅ Get worksheet from fileGroup (may change after flush)
        const currentWorksheet = fileGroup.worksheets[cleanedSheetName];
        if (!currentWorksheet) {
          throw new Error(
            `Worksheet ${cleanedSheetName} not found during processing`,
          );
        }
        ...
        // Check if need to flush to new part
        if (rowCount % this.batchSize === 0) {
          await this.flushToNewPart(groupId, cleanedSheetName);
        }
      }
    } else {
      ...
      >) {
        ...
        if (currentRowCount % this.batchSize === 0) {
          await this.flushToNewPart(groupId, cleanedSheetName);
        }
  ...
  private async flushToNewPart(
    ...
    if (fileGroup.currentTempPath) {
      ...
      console.log(
        `[StreamExporter] Flushed part ${fileGroup.partIndex} to disk: ${fileGroup.currentTempPath} (${fileGroup.rowCount[sheetName]} rows)`,
      );
    }
    ...
    // Re-create worksheet with same columns
