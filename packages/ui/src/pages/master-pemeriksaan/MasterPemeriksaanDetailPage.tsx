...
export default function MasterPemeriksaanDetailPage() {
    ...
    <AppLayout>
      <div className="ui-space-y-6">
        <div className="ui-flex ui-justify-between ui-items-center">
          <h5 className="ui-font-bold ui-text-xl">{t('master-pemeriksaan:title.detail')}</h5>
          <div className="ui-flex ui-gap-2">
            <Button variant="outline" onClick={() => router.back()}>
              {t('common:back')}
            </Button>
            <Button onClick={() => router.push(`/v5/master-pemeriksaan/${id}/edit`)}>
              {t('common:edit')}
            </Button>
          </div>
        </div>
        ...
        <div className="ui-bg-white ui-p-6 ui-rounded-lg ui-border ui-border-neutral-200">
          <dl className="ui-space-y-6">
            <DetailField label={t('master-pemeriksaan:form.name.label')} value={data.name} />

            <DetailField
              label={t('master-pemeriksaan:form.jenis_pemeriksaan.label')}
              value={data.examination_type_name}
            />

            <DetailField
              label={t('master-pemeriksaan:table.description')}
