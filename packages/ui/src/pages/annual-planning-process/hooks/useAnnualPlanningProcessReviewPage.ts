...
import { AnnualPlanningProcessForm, AnnualPlanningProcessFormKey, UpdateStatusAnnualPlanningProcessPopulationBody, UserTag } from "../annual-planning-process.types"
import { ENTITY_TYPE } from "#constants/entity"
import { useForm } from "react-hook-form"
import { fetchGroupTarget, setFormDataPopulationTarget, setFormUsageIndex, setInitialValuesForm } from "../annual-planning-process.utils"
import { useParams } from "next/navigation"
import { useMutation, useQuery } from "@tanstack/react-query"
import { getDetailAnnualPlanningProcess, updateStatusAnnualPlanningProcessPopulation, createAnnualPlanningProcessResult } from "../annual-planning-process.services"
...
export const useAnnualPlanningProcessReviewPage = () => {
  ...
  const { mutateCentralPopulation } = useInformationPopulationTarget()
  ...
  const onMutateAnnualPlanningProcessPopulation = async (data: AnnualPlanningProcessForm['population_correction']) => {
    ...
    if (data && area_program_plan?.id) {
      const payload: UpdateStatusAnnualPlanningProcessPopulationBody[] = []

      data.forEach(population => {
        population?.data?.forEach(target => {
          payload.push({
            id: target.id ?? 0,
            entity_id: population.entity_id ?? 0,
      ...
      return updateStatusAnnualPlanningProcessPopulation(Number(area_program_plan.id), payload)
    }
  }
