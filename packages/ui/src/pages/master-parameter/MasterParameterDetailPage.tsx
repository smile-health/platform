...
export default function MasterParameterDetailPage() {
      ...
      <div className="ui-space-y-6">
        <div className="ui-flex ui-justify-between ui-items-center">
          <h5 className="ui-font-bold ui-text-xl">
            {t('master-parameter:title.detail')}
          </h5>
          <div className="ui-flex ui-gap-2">
            <Button variant="outline" onClick={() => router.back()}>
              {t('common:back')}
            </Button>
            <Button onClick={() => router.push(`/v5/master-parameter/${id}/edit`)}>
              {t('common:edit')}
            </Button>
          </div>
        </div>
        ...
        <div className="ui-bg-white ui-p-6 ui-rounded-lg ui-border ui-border-neutral-200">
          <dl className="ui-space-y-6">
            <DetailField
              label={t('master-parameter:form.name.label')}
              value={data.name}
            />

            <DetailField
              label={t('master-parameter:form.unit.label')}
              value={data.unit || '-'}
            />

            <DetailField
              label={t('master-parameter:form.description.label')}
