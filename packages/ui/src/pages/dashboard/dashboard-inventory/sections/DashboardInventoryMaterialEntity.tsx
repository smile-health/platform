...
import useDashboardIOTMaterialEntity from '../../hooks/useDashboardIOTMaterialEntity'
...
const DashboardInventoryMaterialEntity = forwardRef<
  TDashboardIOTHandler,
  Props
>(({ type, filter, enabled = false }, ref) => {
  ...
  const isSingleColumn = type === 'abnormal-stock' || type === 'filling-stock'
  ...
  const { dataSource, isLoading } = useDashboardIOTMaterialEntity(ref, {
    path: type,
    filter,
    page,
    paginate,
...
DashboardInventoryMaterialEntity.displayName =
  'DashboardInventoryMaterialEntity'

export default DashboardInventoryMaterialEntity
