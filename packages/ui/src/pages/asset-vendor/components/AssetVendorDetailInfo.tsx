...
import { AssetVendorDetailInfoProps } from '../asset-vendor.type'
import { generateDetail } from '../utils/helper'
import AssetVendorSkeleton from './AssetVendorSkeleton'

export default function AssetVendorDetailInfo(
  props: AssetVendorDetailInfoProps
) {
  ...
  const getEditUrl = () => {
    if (isGlobal) {
      return getAsLinkGlobal(
        `/v5/global-settings/asset/vendor/${data?.id}/edit`,
        null,
        {
          fromPage: 'detail',
        }
    ...
    return getAsLink(`/v5/asset/vendor/${data?.id}/edit`, null, {
      fromPage: 'detail',
    })
  }
  ...
  return (
    <div className="ui-p-4 ui-mt-6 ui-border ui-border-neutral-300 ui-rounded ui-space-y-4">
      <ModalConfirmation
        ...
        description={
          status
            ? t('assetVendor:action.deactivate.confirmation')
            : t('assetVendor:action.activate.confirmation')
        }
        onSubmit={() =>
          onUpdateStatus({
            id: String(data?.id),
            status: data?.status?.id ? 0 : 1,
          })
        }
      />
      ...
      {/* <ActiveLabel isActive={Boolean(data?.status?.id)} /> */}
