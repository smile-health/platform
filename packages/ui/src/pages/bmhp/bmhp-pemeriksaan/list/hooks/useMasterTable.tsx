...
// import BmhpHistoryStatusCapsule from '#pages/bmhp/bmhp-history/components/BmhpHistoryStatus'
...
import { deleteBmhpPemeriksaan, TBmhpPemeriksaanData } from '../master.service'

export interface PemeriksaanTableProps {
  basePath: string
}

const useMasterTable = (props: PemeriksaanTableProps) => {
  ...
  const mutation = useMutation({
    mutationFn: async (id: number) => {
      return await deleteBmhpPemeriksaan(id)
    },
    onSuccess: () => {
  ...
  const columns: ColumnDef<TBmhpPemeriksaanData>[] = useMemo(
    () => [
      {
      ...
      {
        header: t('label.examination_type'),
        accessorKey: 'examination_type_name',
        enableSorting: true,
        size: 200,
        minSize: 150,
        cell: ({ row }) => {
          return <div>{row.original.examination_type_name || '-'}</div>
        },
      },
      // {
      ...
      //     return (
      //       <BmhpHistoryStatusCapsule status={isActive ? 'ACTIVE' : 'INACTIVE'} />
      //     )
      //   },
      // },
      {
