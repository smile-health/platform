...
import { deleteBmhpVariant, TBmhpVariantData } from '../master.service'
...
const useMasterTable = (props: VariantTableProps) => {
  ...
  const mutation = useMutation({
    mutationFn: async (id: number) => {
      return await deleteBmhpVariant(id)
    },
    onSuccess: () => {
  ...
  const columns: ColumnDef<TBmhpVariantData>[] = useMemo(
    () => [
      {
      ...
      {
        header: t('label.material'),
        accessorKey: 'material_name',
        enableSorting: true,
        size: 250,
        minSize: 200,
        cell: ({ row }) => {
          return <div>{row.original.material_name || '-'}</div>
        },
      },
      {
