...
import {
  deleteBmhpExaminationType,
  TBmhpExaminationTypeData,
} from '../master.service'
...
const useMasterTable = (props: MethodTableProps) => {
  ...
  const mutation = useMutation({
    mutationFn: async (id: number) => {
      return await deleteBmhpExaminationType(id)
    },
    onSuccess: () => {
  ...
  const columns: ColumnDef<TBmhpExaminationTypeData>[] = useMemo(
    () => [
      {
      ...
      },
      {
        ...
        minSize: 300,
        cell: ({ row }) => {
          return <div className="ui-truncate">{row.original.description}</div>
        },
      },
      {
