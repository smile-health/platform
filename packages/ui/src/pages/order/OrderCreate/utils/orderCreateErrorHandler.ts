...
export const handleOrderErrors = (
  ...
  if (orderItemsErrors && typeof orderItemsErrors === 'object') {
        ...
        for (const fieldKey in fieldErrors) {
          ...
          if (Array.isArray(errorMessages)) {
            ...
            if (
              ...
              setError(
                `order_items.${Number(index)}.value.${fieldKey}` as keyof TOrderCreateForm,
                {
    ...
    for (const index in combinedErrors) {
      setError(
        `order_items.${Number(index)}.value.ordered_qty` as keyof TOrderCreateForm,
        {
  ...
  Object.entries(response.errors).forEach(([key, errorValue]) => {
    if (key !== 'order_items') {
      setError(key as keyof TOrderCreateForm, {
