...
export const listBmhpPlanningYears = async (
  ...
  const fetchYearList = await axios.get('/main/annual-planning/program-plans', {
    params,
    cleanParams: true,
  })
...
export const markFinalBmhpPlanning = async (id: number) => {
  const response = await axios.put(`/main/annual-planning/program-plans/${id}`, null, {
  })
...
export const getExistingYears = async (): Promise<number[]> => {
  const response = await axios.get('/main/annual-planning/program-plans', {
    params: {
      page: 1,
      paginate: 50,
    },
