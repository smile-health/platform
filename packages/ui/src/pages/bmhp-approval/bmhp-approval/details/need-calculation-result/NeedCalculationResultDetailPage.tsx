...
import { useNeedCalculationResult } from '../../tabs/need-calculation-result/hooks/useNeedCalculationResult'
import { TNeedCalculationResultItem } from '../../tabs/need-calculation-result/libs/need-calculation-result.type'
import {
  exportMaterialNeeds,
  loadMaterialNeedsEntities,
} from '../../tabs/need-calculation-result/services/need-calculation-result.service'
...
const NeedCalculationResultDetailPageContent: React.FC = () => {
  const { t, i18n } = useTranslation(['bmhpApproval', 'common'])
  const { query } = useSmileRouter() as {
    query: { year_id?: string; regency_id?: string }
  }
  ...
  const filterSchema = useMemo(
    () => [
      {
        id: 'need_calc_detail_material_filter',
      ...
      {
        id: 'need_calc_detail_entity_filter',
        type: 'select-async-paginate' as const,
        name: 'entity',
        ...
        additional: { page: 1, entity_regency_id: query?.regency_id },
      },
    ],
    [t, profile?.entity?.regency?.id]
  )
  ...
  const params = useMemo(
    ...
    [yearFromUrl, query.regency_id, appliedFilters, pagination]
  )
  ...
  const { data, isLoading, isFetching } = useNeedCalculationResult({
    params,
    enabled: !!yearFromUrl && hasMaterialFilter,
  })
  ...
  const handleFilterReset = useCallback(() => {
    filter.reset()
    setAppliedFilters({ materialId: undefined, entityId: undefined })
    setPagination({ page: 1 })
  }, [filter, setPagination])
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      await exportMaterialNeeds({
        program_plan_id: yearFromUrl ?? 0,
        regency_id: query.regency_id ? Number(query.regency_id) : undefined,
        entity_id: appliedFilters.entityId,
        material_id: appliedFilters.materialId,
      })
    } finally {
  ...
  }, [yearFromUrl, query.regency_id, appliedFilters])

  // ── Derive table data (per material) ─────────────────────────────────────
      ...
      <div className="ui-border ui-p-5 ui-space-y-4">
        ...
        <div className="ui-w-full ui-overflow-x-auto ui-border ui-border-gray-200 ui-rounded">
          <table className="ui-w-full ui-text-sm ui-border-collapse">
            <thead>
              {/* Row 1: No + Puskesmas (rowspan=2) + material group headers */}
              ...
              {/* Row 2: sub-column headers per material group */}
              <tr className="ui-bg-gray-50 ui-border-b ui-border-gray-200">
                {materialGroups.map((mg) => (
              ...
              {tableRows.length === 0 && !isLoading && !isFetching ? (
                ...
                tableRows.flatMap((row) =>
                  Array.from({ length: row.maxMaterials }, (_, mi) => (
                    ...
                    >
                      {/* No + Nama — only on first sub-row with rowSpan */}
                      {mi === 0 && (
                        <>
                          <td
                      ...
                      {/* Per material group — material at index mi */}
                      {materialGroups.map((mg) => {
                        const materials =
                          row.screeningMap.get(mg.material_name) ?? []
...
const NeedCalculationResultDetailPage: React.FC = () => {
  return (
    <BmhpApprovalDetailTabs>
      <NeedCalculationResultDetailPageContent />
    </BmhpApprovalDetailTabs>
  )
}

export default NeedCalculationResultDetailPage
