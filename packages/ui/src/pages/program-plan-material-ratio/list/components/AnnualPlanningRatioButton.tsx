...
export const AddRatioButton = ({ href }: { href: string }) => {
  ...
  return (
    <Button asChild leftIcon={<Plus className="ui-size-5" />}>
      <Link href={href}>{t('add')}</Link>
    </Button>
  )
}

export const ImportRatioButton = ({ onClick }: { onClick: () => void }) => {
  const { t } = useTranslation('common')
  const { detailProgramPlanData } = useContext(ProgramPlanDetailContext)
...
export const DownloadTemplateRatioButton = ({
  onClick,
}: {
  onClick: () => void
}) => {
