...
import { useMaterialVolumeList } from './hooks/useMaterialVolumeList'
import useMaterialVolumeTable from './hooks/useMaterialVolumeTable'

const MaterialVolumeListPage: React.FC<CommonType> = ({ isGlobal }) => {
  ...
  } = useMaterialVolumeList({ isGlobal })
  const { schema: tableSchema } = useMaterialVolumeTable({
    isGlobal,
  ...
  return (
    <Container
      title={t('materialVolume:title.list')}
      hideTabs={isGlobal}
      withLayout={!isGlobal}
    >
      <Meta
        title={`Smile | ${isGlobal ? 'Global' : ''} ${t('materialVolume:title.list')}`}
      />
...
export default MaterialVolumeListPage
