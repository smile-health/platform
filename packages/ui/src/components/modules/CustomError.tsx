...
import BadGatewayIcon from '#components/icons/BadGatewayIcon'
import ErrorConnectionIcon from '#components/icons/ErrorConnectionIcon'
import ForbiddenIcon from '#components/icons/ForbiddenIcon'
import GatewayTimeoutIcon from '#components/icons/GatewayTimeoutIcon'
import NotFoundDataIcon from '#components/icons/NotFoundDataIcon'
import NotFoundPagesIcon from '#components/icons/NotFoundPagesIcon'
...
const CustomError: React.FC<Props> = ({
  ...
  const handleAction = () => {
    if (errorState[currentError].action === 'redirect')
      window.location.replace(`/${language}/v5/program`)
  ...
  return (
    <Container title="" withLayout={withLayout} hideTabs={hideTabs}>
      <Meta title={`SMILE | ${errorState[currentError].title}`} />
